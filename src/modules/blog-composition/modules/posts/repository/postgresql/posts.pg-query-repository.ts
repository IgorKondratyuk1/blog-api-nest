...
export class PostsPgQueryRepository extends PostsQueryRepository {
  ...
  public async findAll(queryObj: QueryDto, currentUserId: string = null): Promise<PaginationDto<ViewPostDto>> {
    ...
    const queryTotalCount = `SELECT count(*) FROM public.post pt b LEFT JOIN public.blog bt ON pt.blog_id = bt.id ${filters};`;
  ...
  async findPostsOfBlog(
    ...
    let findPostQuery =
      ...
      'FROM public.post pt ' +
      'LEFT JOIN public.blog bt ON pt.blog_id = bt.id ' +
      'WHERE pt.blog_id=$1 AND pt.is_banned = FALSE';
  ...
  async findPostsOfBlogByUserId(
    ...
    let findPostQuery =
      ...
      'FROM public.post pt ' +
      'LEFT JOIN public.blog bt ON pt.blog_id = bt.id ' +
      'WHERE pt.blog_id=$1 AND pt.user_id=$2 AND pt.is_banned = FALSE ';
    findPostQuery += filters;
    findPostQuery += ` ORDER BY "${queryObj.sortBy}" ${sortValue}`;
  ...
  getFilters = (queryObj: QueryDto, searchBannedPosts: boolean): string => {
    const sqlFilters = [];

    // TODO: check if acronim banId is work
    if (searchBannedPosts) {
      sqlFilters.push('is_banned = TRUE');
    } else {
      sqlFilters.push('is_banned = FALSE');
    }
