...
describe('/blogs', () => {
  ...
  it('PUT: blog should be updated by correct data', async () => {
    ...
    await request(baseUrl).put(`/blogs/${firstBlog.id}`).set('Authorization', basicAuthValue).send(data).expect(204);
  ...
  it('PUT: blog shouldn`t be updated by wrong data (without wrong id)', async () => {
    ...
    await request(baseUrl).put(`/blogs/1234556789`).set('Authorization', basicAuthValue).send(data).expect(404);
  });

  // DELETE
  it('DELETE: blog shouldn`t be deleted (without wrong id)', async () => {
    await request(baseUrl).delete(`/blogs/1234556789`).set('Authorization', basicAuthValue).expect(404);
  });

  it('DELETE: blog should be deleted', async () => {
    await request(baseUrl).delete(`/blogs/${firstBlog.id}`).set('Authorization', basicAuthValue).expect(204);

    await request(baseUrl).delete(`/blogs/${secondBlog.id}`).set('Authorization', basicAuthValue).expect(204);
  });
