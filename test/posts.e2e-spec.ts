...
describe('/blogs/:blogId/posts', () => {
  ...
  it('GET: should return correct posts of blog', async () => {
    const result = await request(baseUrl)
      .get(
        `/blogs/${arrOfBlogs[0].id}/posts?pageNumber=1&pageSize=3&sortBy=createdAt&sortDirection=desc`,
      )
      .expect(200);
  ...
  it('GET: should return created posts of blog', async () => {
    const result = await request(baseUrl)
      .get(
        `/blogs/${arrOfBlogs[0].id}/posts?pageNumber=4&pageSize=3&sortBy=createdAt&sortDirection=desc`,
      )
      .expect(200);
